  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        >←</button>
        <div>
          <h2 className="text-xl font-bold text-gray-800">Harvest Logger</h2>
      ...
      {readyBatches.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-lg font-bold text-gray-700 mb-1">Nothing ready yet</h3>
          <p className="text-sm text-gray-500">Check back when batches reach the Ready stage.</p>
          <button
            onClick={() => navigate('/production')}
        ...
        <div className="space-y-4">
          {readyBatches.map((batch) => {
            ...
            return (
              <div
                key={batch.id}
                className="bg-white rounded-2xl border-2 border-green-300 p-5 shadow-sm"
              >
                {/* Batch info */}
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="font-bold text-gray-800 text-base">{batch.varietyName}</h3>
                    <p className="text-sm text-gray-500 mt-0.5">
                      {batch.quantity} {batch.unit}s · {catLabel}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">Sown {batch.sowDate}</p>
                  </div>
