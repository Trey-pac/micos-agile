...
export default function DeliveryTracker({ deliveries = [], loading = false }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-gray-800">🚚 Delivery Tracker</h2>
        <p className="text-sm text-gray-500">EasyRoutes integration — real-time delivery progress</p>
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Today's Routes", value: todayRoutes.length, color: 'text-gray-800' },
          { label: 'Active Now',     value: activeCount,        color: activeCount > 0 ? 'text-blue-700' : 'text-gray-400' },
          { label: 'Stops Delivered', value: deliveredStops,     color: deliveredStops > 0 ? 'text-green-700' : 'text-gray-400' },
          { label: 'Total Stops',    value: totalStops,          color: 'text-gray-600' },
      ...
      {sorted.length === 0 ? (
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-700 mb-1">No active deliveries</h3>
          <p className="text-sm text-gray-400">Dispatch routes from EasyRoutes to see them here</p>
        </div>
      ) : (
...
function RouteCard({ delivery, todayStr }) {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border overflow-hidden ${
      isToday ? 'border-sky-200' : 'border-gray-100'
    }`}>
      {/* Card header */}
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-gray-400">{delivery.date} · {stops.length} stop{stops.length !== 1 ? 's' : ''}</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-lg font-black text-gray-800">{pct}%</div>
          <div className="text-[10px] text-gray-400 font-semibold">{deliveredCount}/{stops.length}</div>
        </div>
      </div>
      ...
      <div className="h-1.5 bg-gray-100">
        <div
          className={`h-full transition-all duration-500 rounded-full ${
            pct === 100 ? 'bg-green-500' : pct > 0 ? 'bg-sky-500' : 'bg-gray-200'
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
  ...
  return (
    <div className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-gray-700 truncate">{stop.customerName || 'Unknown'}</div>
        {stop.orderId && (
          <div className="text-[10px] text-gray-400 truncate">Order: {stop.orderId}</div>
        )}
      </div>
      ...
      {stop.proofPhotoUrl && (
        ...
        >
          <img
            ...
            alt="Proof"
            className="w-8 h-8 rounded-lg object-cover border border-gray-200 hover:border-sky-400 transition-colors"
          />
        </a>
      )}
    </div>
