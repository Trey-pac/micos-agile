...
import { AlertCircle, AlertTriangle, CheckCircle, Sprout, BarChart3 } from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Sowing Schedule</h2>
        ...
        <div className="flex gap-2">
          {critCount > 0 && (
            <Badge variant="destructive">
              <AlertCircle className="w-3 h-3 mr-1" /> {critCount} Critical
            </Badge>
          )}
          {warnCount > 0 && (
            <Badge variant="warning">
              <AlertTriangle className="w-3 h-3 mr-1" /> {warnCount} Low Stock
            </Badge>
          )}
      ...
      <Tabs value={tab} onValueChange={setTab} className="mb-5">
        <TabsList>
          <TabsTrigger value="recs"><Sprout className="w-3 h-3 mr-1" /> Recommendations</TabsTrigger>
          <TabsTrigger value="pipeline"><BarChart3 className="w-3 h-3 mr-1" /> Pipeline</TabsTrigger>
        </TabsList>
          ...
          {visible.map((need) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-gray-800 dark:text-gray-100">{need.cropName}</p>
                    <Badge variant="outline" className={`${URGENCY_STYLE[need.urgency]}`}>
                ...
                <div className="text-right shrink-0">
                  <p className="text-lg font-bold text-gray-800 dark:text-gray-100">{need.recommendedQty}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{need.batchUnit}s to plant</p>
                </div>
              </div>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                >
                  <Button disabled={planting === need.cropId} className="w-full">
                    {planting === need.cropId ? 'Planting…' : <><Sprout className="w-4 h-4 mr-1" /> Plant Now</>}
                  </Button>
                </motion.button>
                <Button
                  variant="outline"
      ...
      <TabsContent value="pipeline">
        <Card className="rounded-2xl">
          ...
          <h3 className="font-bold text-gray-700 dark:text-gray-200 text-sm mb-4">Days of Supply — Active Pipeline</h3>
          {sowingNeeds.length === 0 ? (
            <p className="text-gray-400 dark:text-gray-500 text-sm text-center py-8">No demand data yet.</p>
          ) : (
                ...
                return (
                  <div key={need.cropId}>
                    <div className="flex justify-between text-sm mb-1">
                        ...
                        {need.cropName}
                        {need.autoGenerated && (
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-sky-100 text-sky-700">⚡ Auto</span>
                        )}
                      </span>
          ...
          {/* Legend */}
          <div className="flex gap-4 mt-5 pt-4 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
