/**
 * shopifyAdmin.js — Shopify Admin API (GraphQL) service layer.
 *
 * Lives in api/_lib/ so Vercel serverless functions can import it.
...
function getConfig() {
  ...
  if (!domain) console.error('[shopify] SHOPIFY_STORE_DOMAIN is not set');
  if (!token) console.error('[shopify] SHOPIFY_ADMIN_API_TOKEN is not set');
  if (token) console.log('[shopify] Token present, length:', token.length, 'starts with:', token.substring(0, 6) + '...');
  if (domain) console.log('[shopify] Domain:', domain);
...
async function shopifyGraphQL(query, variables = {}) {
  ...
  // Always read the raw body first — safe regardless of content type
  const rawText = await res.text();

  if (!res.ok) {
    console.error(`[shopify] HTTP ${res.status} response:`, rawText.substring(0, 500));
    throw new Error(`Shopify API ${res.status}: ${rawText.substring(0, 300)}`);
  }

  // Parse JSON from the raw text (not res.json() which can't be called after res.text())
  let json;
  try {
    json = JSON.parse(rawText);
  } catch (parseErr) {
    console.error('[shopify] Invalid JSON response:', rawText.substring(0, 500));
    throw new Error(`Shopify returned non-JSON (${res.status}): ${rawText.substring(0, 200)}`);
  }
...
async function fetchAllPages(query, connectionPath, pageSize = 50) {
  ...
  while (hasNextPage) {
    page++;
    console.log(`[shopify] Fetching ${connectionPath} page ${page}...`);
    ...
    if (!connection || !connection.edges) {
      console.error('[shopify] No connection/edges at path:', connectionPath, 'data:', JSON.stringify(data).substring(0, 300));
      throw new Error(`No data at path "${connectionPath}" — check query`);
    }
  ...
  console.log(`[shopify] Fetched ${allNodes.length} total ${connectionPath}`);
  return allNodes;
}
